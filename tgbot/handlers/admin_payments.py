...
"""
    ...
    if way == "qiwi":
    ...
    elif way in ["btc", "ETH", "USDT_ERC20", "USDC_ERC20", "TRX", "USDT_TRC20", "USDC_TRC20", "BNB", "USDT_BEP20", "BUSD", "SOL", "USDC_SOL", "LTC"]:
        ways = {        
        "btc": "BTC",
        "ETH": "ETH",
        ...
        status = get_payments()[f"pay_{way}"]
        address = get_payments()[f"{way}_adrs"] or "❌ Адрес не задан"

        msg = f"""
<b>{ways[way]}

Статус: <code>{'✅ Включен' if status == 'True' else '❌ Выключен'}</code>
Адрес: <code>{address}</code></b>
"""
        await call.message.edit_text(msg, reply_markup=payments_settings_info(way, status))
...
async def off_payments(call: CallbackQuery, state: FSMContext):
    ...
    if way == "qiwi":
        ...
        if action == "off":
            update_payments(**{f"pay_{way}": "False"})
        else:
            update_payments(**{f"pay_{way}": "True"})
...
@dp.callback_query_handler(IsAdmin(), text_startswith="payments_edit_address:", state="*")
async def edit_crypto_address(call: CallbackQuery, state: FSMContext):
    way = call.data.split(":")[1]

    if way in ["btc", "ETH", "USDT_ERC20", "USDC_ERC20", "TRX", "USDT_TRC20", "USDC_TRC20", "BNB", "USDT_BEP20", "BUSD", "SOL", "USDC_SOL", "LTC"]:
        await state.update_data(crypto_way=way)
        await call.message.answer(f"Введите новый адрес для {way.upper()}:")
        await state.set_state("waiting_crypto_address")

@dp.message_handler(state="waiting_crypto_address")
async def set_crypto_address(msg: Message, state: FSMContext):
    data = await state.get_data()
    way = data.get("crypto_way")

    if way:
        update_payments(**{f"{way}_adrs": msg.text})
        await state.finish()
        await msg.answer(f"✅ Адрес {way.upper()} обновлён: <code>{msg.text}</code>", parse_mode="HTML")       
